...
def check_valid_config(config):
    if not (config and config.model_config and config.serving_config and
            config.tokenizer and config.basic_inputs and config.extra_inputs and config.warmup_inputs):
        return False
    # model_path校验
    if config.model_config.backend != "kbk":
    ...
    if model_config.decode_batch_size[0] != 1:
        model_config.seq_length=[520]
        config.pa_config['decode_seq_length'] = 520
    #   # pa_config校验
    ...
    if config.model_config.backend == "ge" or config.model_config.backend is None:
