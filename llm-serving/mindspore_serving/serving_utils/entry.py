...
class EntryStatus(enum.Enum):
    # 表示生成任务的状态
    """Status of a entry."""
    WAITING = enum.auto()  # waiting for inference等待推理
    RUNNING = enum.auto()  # doing inference推理中
    WAITING_BATCH = enum.auto()  # just using in static batch静态批处理中
    FINISHED_STOPPED = enum.auto()  # inference over, request down推理完成，请求结束
    FINISHED_LENGTH_CAPPED = enum.auto()  # over max_toekn_len超过最大token长度
    FINISHED_ABORTED = enum.auto()  # expection异常中止
    FINISHED_IGNORED = enum.auto()  # ingore this request忽略该请求
    PADDING_INVAILED = enum.auto()  # padding is invaild填充无效
    INPUT_OUTOFRANGE = enum.auto()# 输入超出范围
    EMPTY_PROMPT_TOKEN = enum.auto()# 空提示token
...
class EntryData:
    # 用于存储和管理单个生成任务的数据，包括输入token、生成的输出token、任务状态等。
    def __init__(self,
...
class Entry:
    # 用于封装一次生成任务，包含了生成任务的输入文本、任务状态、生成的token及其对数概率等。
    def __init__(self,
                 prompt: str,
                 prompt_tokens: List[int],
                 max_token_len):
...
class EntryMetaData:
    """
       ...
       用来封装生成任务的元数据，通常用于批处理和任务管理场景。它结合了任务的输入、状态以及元数据，用于控制和调度生成任务。
    """
    ...
    def __lt__(self, other):
        return self.entry_data.get_max_token_len() > other.entry_data.get_max_token_len()
